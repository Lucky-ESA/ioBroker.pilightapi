...
class Pilightapi extends utils.Adapter {
    ...
    async jsonEvaluate(JsonStr) {
        ...
        try {
             ...
             //if(jsonMessage.origin !== "core") this.log.info("Jsonparse: " + JSON.stringify(jsonMessage));
             ...
             if(jsonMessage.origin === "update" && this.sentiob === 0) {
                       ...
                       if(n === "values") {
                            Object.keys(jsonMessage[n]).forEach((v) => {
                                 ...
                                 this.getForeignObject(this.namespace + ".devices." + jsonMessage["devices"] + "." + v, async (err, obj) => {
                                      let statebol = jsonMessage[n][v];
                                      if(statebol === "false") {
                                           statebol = false;
                                      } else if(statebol === "true") {
                                           statebol = true;
                                      }
                                      if (!obj || !obj.common) {
                                           ...
                                           this.setState(this.namespace + ".devices." + jsonMessage["devices"] + "." + v, statebol, true);
                                      } else {
                                           this.setState(this.namespace + ".devices." + jsonMessage["devices"] + "." + v, statebol, true);
                                      }
                                 });
                  ...
                  return true;
             } else if(jsonMessage.origin === "update" && this.sentiob === 1) {
                  this.log.debug("Receive input from iobroker!");
                  this.sentiob = 0;
                  return true;
             }
        } catch (err) {
